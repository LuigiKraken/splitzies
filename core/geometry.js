...
export const pointInRect = (x, y, r) => x >= r.left && x <= r.left + r.width && y >= r.top && y <= r.top + r.height;
...
export function pointInPolygon(x, y, poly) {
  ...
  for (let i = 0, j = poly.length - 1; i < poly.length; j = i, i++) {
    ...
    if (((yi > y) !== (yj > y)) && (x < ((xj - xi) * (y - yi)) / ((yj - yi) || 1e-8) + xi)) {
      inside = !inside;
    }
  }
  return inside;
...
export function getCenterRect(bounds, config) {
...
export function getEffectiveLayerCount(bounds, depth, config) {
...
function intersectWithVertical(a, b, xLine) {
  const dx = b.x - a.x;
...
function intersectWithHorizontal(a, b, yLine) {
  const dy = b.y - a.y;
...
function getDirectionalBasePolygon(bounds, direction) {
  ...
  const l = bounds.left, r = bounds.left + bounds.width;
  const t = bounds.top, b = bounds.top + bounds.height;

  if (direction === "TOP")    return [{ x: l, y: t }, { x: r, y: t }, { x: cx, y: cy }];
  if (direction === "BOTTOM") return [{ x: r, y: b }, { x: l, y: b }, { x: cx, y: cy }];
  if (direction === "LEFT")   return [{ x: l, y: b }, { x: l, y: t }, { x: cx, y: cy }];
  return [{ x: r, y: t }, { x: r, y: b }, { x: cx, y: cy }];
}

function clipAlongAxis(poly, min, max, isHorizontal) {
  const intersect = isHorizontal ? intersectWithHorizontal : intersectWithVertical;
...
export function getDisplayDirectionalBandPolygon(bounds, layer, totalLayers, direction, startRatio) {
...
export function polygonToClipPath(bounds, poly) {
  ...
  return `polygon(${poly.map((p) =>
    `${clamp(((p.x - bounds.left) / bounds.width) * 100, 0, 100)}% ${clamp(((p.y - bounds.top) / bounds.height) * 100, 0, 100)}%`
  ).join(", ")})`;
}
